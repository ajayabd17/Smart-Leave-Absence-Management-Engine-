document.addEventListener('DOMContentLoaded', () => {
    ...
    function isHrPendingItem(row) {
        ...
        if (status === 'REJECTED' || status === 'APPROVED' || status === 'AUTO_REJECTED') return false;
    ...
    function setupHrNotifications(authPayload) {
        ...
        function statusClassFromType(eventType) {
            ...
            if (eventType === 'DECISION_APPROVED') return 'status-approved';
            if (eventType === 'DECISION_REJECTED') return 'status-rejected';
        ...
        function titleFromEvent(item) {
            ...
            if (item.event_type === 'DECISION_APPROVED') return `You approved request ${item.request_id}`;
            if (item.event_type === 'DECISION_REJECTED') return `You rejected request ${item.request_id}`;
        ...
        window.__appendHrDecisionNotification = (requestId, decision, context = {}) => {
            appendEvent({
                event_type: decision === 'APPROVED' ? 'DECISION_APPROVED' : 'DECISION_REJECTED',
    ...
    async function loadPendingApprovals() {
        ...
        const hrRows = rows.filter(isHrPendingItem);
        if (typeof window.__setHrPendingNotifications === 'function') {
            window.__setHrPendingNotifications(hrRows);
        }

        document.getElementById('stat-pending').textContent = String(hrRows.length);
    ...
    async function handleDecision(button, decision) {
        ...
        const itemNode = button.closest('.approval-item');
        const requestText = itemNode?.querySelector('.requester-info p')?.textContent || '';
        const leaveType = (requestText.split('(')[0] || '').trim();
